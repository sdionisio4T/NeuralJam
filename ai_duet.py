...
from neuraljam.config import MIDI_INPUT_NAME, MIDI_OUTPUT_NAME
...
# === HELPERS MIDI ===
def find_port_by_prefix(prefix, ports):
    for p in ports:
        if p.startswith(prefix):
            return p

    raise RuntimeError(f"No se encontró puerto MIDI con prefijo: {prefix}")
...
# === MAIN ===
def main():
    input_name = find_port_by_prefix(MIDI_INPUT_NAME, mido.get_input_names())

    output_name = find_port_by_prefix(MIDI_OUTPUT_NAME, mido.get_output_names())

    print(f"Puerto input detectado:  {input_name}")
    print(f"Puerto output detectado: {output_name}")
