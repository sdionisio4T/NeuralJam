...
def main():
    ...
    from neuraljam.midi import PhraseDetector, MidiOutput
    ...
    try:
        detector.start()
        midi_out.open()
        log.info("Sistema listo. Tocá una frase y esperá la respuesta. Ctrl+C para salir.")
        log.info(
        ...
        while True:
            ...
            log.info(
                f"--- Turno {turn} --- {len(phrase.notes)} notas, "
                f"{total_dur:.2f}s"
            )
